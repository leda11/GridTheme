...
//-------------------------------------------------------------------------------
      ...
      public function GetHTML($attributes=null) {
        ...
        $method = " method='post'";
       // MOs har en annan variant med 'start' -tror inte den används än. content
...
public function Check() {
    ...
    $values = array();
    if($_SERVER['REQUEST_METHOD'] == 'POST') {
      unset($_SESSION['form-validation-failed']);// mos skriver form-failed
      $validates = true;
      foreach($this->elements as $element) {
        if(isset($_POST[$element['name']])) {
